...
export class Account implements Account{
    ...
    constructor(account: Account = {} as Account){
        ...
        this.name = account.name || "Nova compte";
    ...
    private updateBalance():void{
        const balance = this.entries.reduce((previousValue, currentEntry) => {
            return previousValue + this.amountSegonsTipus(currentEntry);
        }, 0);
        this.balance = balance;
    }

    private amountSegonsTipus(entry: Entry):number{
...
export class Entry implements Entry{
    ...
    constructor( public concept: string, public amount: number, public category: categoryEnum){
        this.id = getRandomId();
